<?php
	include_once '../includes/conn_inc.php';
	include 'admin_includes/admin_exist_inc.php';
	include 'admin_includes/admin_signup_inc.php';
	include 'admin_includes/admin_login_inc.php';
	session_start();
	if(isset($_SESSION['user']))
	{
		header("Location: ../admin/admin_panel_pg");
	}	
?>


<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
	<title>Panou de administrare</title>
</head>
<body>
	<div class="container">
		<div class="jumbotron">	
		
			<?php
				if($resultCheck == 1)
				{
					echo '<h1 class="text-center">Panou de administrare | Log in</h1>';
					echo '<form action="index" method="POST">';
						echo '<div class="form-group"';
							echo '<label for="utilizator">Utilizator:</label>';
							echo '<input type="text" class="form-control" placeholder="Utilizator" id="utilizator" name="userNameLog">';
							echo '<label for="parola">Parola:</label>';
							echo '<input type="password" class="form-control" placeholder="Parola" id="parola" name="userPasswordLog">';
						echo '</div>';
						echo '<button type="submit" class="btn btn-primary" name="submitLogIn">Log in</button>';
					echo '</form>';

				}
				else
				{
					echo '<h1 class="text-center">Panou de administrare | Creeare cont</h1>';
					echo '<br>';
					echo '<p>Aici se poate creea un cont de administrator. Poate exista doar un singur cont de administrator.</p>';
					echo '<form action="index" method="POST">';
						echo '<div class="form-group"';
							echo '<label for="utilizator">Utilizator nou:</label>';
							echo '<input type="text" class="form-control" placeholder="Utilizator" id="utilizator" name="userName">';
							echo '<label for="parola">Parola:</label>';
							echo '<input type="password" class="form-control" placeholder="Parola" id="parola" name="userPassword">';
							echo '<label for="admpsw">Cheie pentru creeare: </label>';
							echo '<input type="password" class="form-control" placeholder="ADMPSW" id="admpsw" aria-describedby="admpswHelp" name="admPsw"> ';
							echo '<small id="admpswHelp" class="form-text text-muted">Pentru a afla cheia, contacteaza creatorul.</small>';
						echo '</div>';
						echo '<button type="submit" class="btn btn-primary" name="submit">Creeaza cont</button>';
					echo '</form>';
				}
			?>
			

		</div>
		
	</div>
	



	<script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
	<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
	<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
</body>
</html>